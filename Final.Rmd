...
for (i in 1:length(k)) {
  ...
  fit_stats_norm[i, "R2"] <- norm_knn_class$R2Pred
  fit_stats_norm[i, "MAE"] <- (ufo$month - norm_knn_class$pred) |> abs() |> mean()
}
...
for (i in 1:length(k)) {
  ...
  fit_stats_stan[i, "R2"] <- stan_knn_class$R2Pred
  fit_stats_stan[i, "MAE"] <- (ufo$month - stan_knn_class$pred) |> abs() |> mean()
}
...
# make the kNN classification tree
knn_classification <- 
  knn(
