...
ufo_filtered <- 
  ufo |> drop_na(longitude, latitude, seconds)
...
for (i in 1:length(k)) {
  loop_knn <- 
    knn.reg(
      train = ufo_norm,
      y = ufo_filtered$seconds,
      k = k[i]
    )
  
  fit_stats_norm[i, "R2"] <- loop_knn$R2Pred
  fit_stats_norm[i, "MAE"] <- (ufo_filtered$seconds - loop_knn$pred) |> abs() |> mean()
}
...
for (i in 1:length(k)) {
  ...
  fit_stats_stan[i, "MAE"] <- (ufo_filtered$seconds - loop_knn$pred) |> abs() |> mean()
}
...
## Making the kNN regression model
``` {r making the knn regression model}
# splitting data into training and testing
knn_train <- sample_frac(ufo_filtered, 0.5)
knn_test <- sample_frac(ufo_filtered, 0.5)
...
# Predicting the duration for ufo data
predict(
  object = tree_full
) ->
  pred_duration
...
bind_rows("predicted" = summary(pred_duration),
          "actual" = summary(knn_test$seconds),
          .id = "income")

knn_train |> mutate(pred_duration = pred_duration) |> 
  summarize(
    "R2_full" = cor(pred_duration, seconds)^2
  )
```
