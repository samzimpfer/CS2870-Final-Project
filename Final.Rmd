...
ufo_raw|>
  ...
  select(datetime, city, state, country, seconds,latitude,longitude, year, month)|>
    ...
    drop_na(longitude, latitude, seconds) -> ufo
...
for (i in 1:length(k)) {
  ...
  fit_stats_norm[i, "MAE"] <- (ufo$seconds - loop_knn$pred) |> abs() |> mean()
}
...
for (i in 1:length(k)) {
  ...
  fit_stats_stan[i, "MAE"] <- (ufo$seconds - loop_knn$pred) |> abs() |> mean()
}
...
knn_train <- sample_frac(ufo, 0.5)
knn_test <- sample_frac(ufo, 0.5)
